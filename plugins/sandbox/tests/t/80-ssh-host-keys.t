...
elsif (@args >= 3 && $args[0] eq '-r' && $args[1] eq '.ssh_keys[] | "github.com " + .') {
    my $keys = load_keys($args[2]);
...
# =====================================================================
{
    ...
    my @bad = grep { !/^github\.com (ssh-ed25519|ssh-rsa|ecdsa-sha2-\S+) \S+$/ } @lines;
    is(scalar(@bad), 0,
       'AC1d: every known-hosts line matches "github.com <keytype> <key>"')
...
#
# INFEASIBILITY NOTE (documented per spec section 3.6's own hedge): with
# the EXACT fixed filter '.ssh_keys[] | "github.com " + .', it is
# analytically impossible for a REAL GitHub response to pass the -e length
# check yet make -r emit zero bytes -- "github.com " is a non-empty literal
# prefix concatenated onto every emitted line, so any array with length > 0
# always yields >= 1 non-empty mapped line (proven live: {"ssh_keys":[""]}
# passes both -e and the write, producing a one-line "github.com " file --
# non-empty, NOT caught by [ -s ... ], but also not a chain-abort case; it
# is a content-shape gap outside the guards' contract, not this AC's target).
#
# So this AC uses the synthetic TEST_JQ_FORCE_EMPTY_R stub flag, modelling
# EXACTLY the failure mode the spec's own inline comment names for this
