...
| `30-connector-hold.t` | `ConnectorHold::should_hold_window` holds a connector's Windows Terminal tab open ONLY on a lost container (nonzero exit + not running), never on a clean quit or a claude error while the container lives; `lost_message` names the container + tells the user the conversation is safe and how to resume; `is_dismiss_key` closes the held window ONLY on Enter (not a stray focus/mouse byte); `terminal_reset_seq` turns off the mouse/focus modes claude left on so focusing the tab can't dismiss it |
...
| `34-claude-json-seed.t` | `ClaudeConfig::heal_claude_json` self-heals claude-home/`.claude.json`'s onboarding bypass: reseeds the template when the file is missing / 0-byte / corrupt (the launcher's old `-f` guard treated a 0-byte file as present, so a truncated config — from an interrupted write or concurrent connector writes — reopened the setup wizard), merges the onboarding keys into a valid config that lost them WITHOUT clobbering other keys, and no-ops (returns undef) on an already-onboarded config |
