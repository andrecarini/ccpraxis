---
name: mergeback-sandboxed-ccpraxis-workcopy
description: Guarded merge-back of the sandboxed ccpraxis work-copy branch into live main. Use when the user has finished reviewing an in-sandbox work-copy run and wants to merge the ~/ccpraxis-sandbox-workcopy branch back into the live ccpraxis. Refuses while a sandbox fleet is live; shows the diff. ALWAYS confirm with the user before committing the merge — never auto-merge.
user-invocable: true
host-only: true
argument-hint: "[--yes]"
...
# Mergeback — Guarded sandboxed-ccpraxis-workcopy merge-back

Merge the sandboxed ccpraxis work-copy branch back into live ccpraxis `main`. The script
...
5. If the user declines at step 2, do nothing (the work-copy is kept for later merge or `/steward:discard-sandboxed-ccpraxis-workcopy`).
