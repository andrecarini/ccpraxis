...
- If `registered: true` → blueprints are now tracked; tell the user to run `/steward:backup` to sync. **Done.**
...
```
If `VAULT_MISSING` → tell the user the vault isn't initialized; run `/steward:backup` first (it prompts for the vault repo URL). Stop.
...
```
perl "${CLAUDE_PLUGIN_ROOT}/scripts/vault-sync.pl" list-orphans
```
...
```
perl "${CLAUDE_PLUGIN_ROOT}/scripts/vault-sync.pl" register --link --cwd "$PROJECT_CWD" --slug "<slug>"
```
...
```
perl "${CLAUDE_PLUGIN_ROOT}/scripts/vault-sync.pl" vault-files --slug "<slug>"
```
...
```
perl "${CLAUDE_PLUGIN_ROOT}/scripts/vault-sync.pl" register --fresh --cwd "$PROJECT_CWD" --slug "<slug>" --files "<files>"
```
...
```
perl "${CLAUDE_PLUGIN_ROOT}/scripts/vault-sync.pl" resolve-conflict --slug "<slug>" --path "<path>" --action <use-local|use-vault|use-merged> --session-id "<session_id>" [--merged-file "<tmp_path>"]
```
...
```
- `committed_and_pushed` → "Registered `<slug>` and synced." Mention any `rolled_back_during_sync`. Future syncs go through `/steward:backup`.
