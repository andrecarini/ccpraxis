...
- Assign `model` per package: `sonnet` default; `opus` for packages with gnarly design surface or security weight. `max_turns` is butler's per-coordinator backstop — this is an **authoring-time** default only (runtime turn budgeting is `b11`'s, untouched here): `max_turns` default 150, and `bp-scout`/other haiku-model dispatches specifically default 40 — repeatedly-observed evidence (`error_max_turns` in 30 of one run's transcripts, three coordinators losing *every* `bp-scout` dispatch to haiku's old 15-turn cap and scouting by hand) showed the old default of 80 (and an unstated haiku default) starved short-lived dispatches. Raise further per package when its own scope clearly needs more.
