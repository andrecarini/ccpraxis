...
sub register_fresh {
    ...
    my $rollback = sub {
        ...
        emit_error("$why — register rolled back; local and vault state restored. Re-run /register-for-backup to try again.");
    };
...
sub register_link {
    ...
    my $rollback = sub {
        ...
        emit_error("$why — link rolled back; local state restored. Vault is untouched. Re-run /register-for-backup to try again.");
    };
...
# register-for-backup skill in link mode to preview what will be pulled
